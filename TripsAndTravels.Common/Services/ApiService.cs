...
namespace TripsAndTravels.Common.Services
{
    public class ApiService : IApiService
    {
        ...
        public async Task<Response> AddNewExpense<T>(string urlBase, string servicePrefix, string controller, string tokenType, string accessToken, T ExpensesRequest)
        {
            try
            {
                string request = JsonConvert.SerializeObject(ExpensesRequest);
                StringContent content = new StringContent(request, Encoding.UTF8, "application/json");
                HttpClient client = new HttpClient
                {
                    BaseAddress = new Uri(urlBase)
